...
class LogtailConfigDetail:
    ...
    :param endpoint : log data endpoint, it should base
        on the region of this project
    ...
    :param log_begin_regex : the regular express to match
    ...
    :param filter_keys_reg : the regex for filter_keys to filter the log,
        filter_keys_reg[i] is for filter_keys[i]. The size of filter_keys_reg
        and filter_keys should be same.
        If a log is matched only if the size of filter_keys is 0, or all the
        value of the related keys in filter_keys,
        match the regex set in filter_keys_reg
    """
    def __init__(self, config_name, logstore_name, endpoint, log_path,
                 file_pattern, log_begin_regex, topic_format,
...
class CommonRegLogConfigDetail(LogtailConfigDetail):
    ...
    :param log_parse_regex : the regular express to match a log,
        e.g (\d+-\d+\d+ \d+:\d+:\d+) (\S+) (.*)

    :type reg_keys : string list
    :param reg_keys : the key for every captured value in
        log_parse_reg, e.g ["time", "level", "message"]
    ...
    :param filter_keys_reg : the regex for filter_keys to filter the log,
        ...
        and filter_keys should be same.
        If a log is matched only if the size of filter_keys is 0, or all the
        value of the related keys in filter_keys, match the
        regex set in filter_keys_reg
    """
    def __init__(self, config_name, logstore_name, endpoint, log_path,
                 file_pattern, time_format, log_begin_regex, log_parse_regex,
                 reg_keys, topic_format="none", filter_keys=[],
                 filter_keys_reg=[]):
        LogtailConfigDetail.__init__(
            self, config_name, logstore_name, endpoint, log_path,
            file_pattern, log_begin_regex, topic_format,
            filter_keys, filter_keys_reg
        )
    ...
    def to_json(self):
        json_value = {}
        json_value["configName"] = self.config_name
        json_value["inputType"] = "file"
        detail = {}
        detail['logType'] = 'common_reg_log'
        detail['logPath'] = self.log_path
        detail['filePattern'] = self.file_pattern
        detail['localStorage'] = True
        detail['timeFormat'] = self.time_format
        detail['logBeginRegex'] = self.log_begin_regex
        detail['regex'] = self.log_parse_regex
        detail['key'] = self.keys
        detail['filterKey'] = self.filter_keys
        detail['filterRegex'] = self.filter_keys_reg
        detail['topicFormat'] = self.topic_format
...
class ApsaraLogConfigDetail(LogtailConfigDetail):
    ...
    :param filter_keys_reg : the regex for filter_keys to filter the log,
        ...
        and filter_keys should be same.
        If a log is matched only if the size of filter_keys is 0, or all the
        value of the related keys in filter_keys, match the regex
        set in filter_keys_reg
    """
    def __init__(
            self, config_name, logstore_name, endpoint, log_path,
            file_pattern, log_begin_regex=r'\[\d+-\d+-\d+ \d+:\d+:\d+\.\d+.*',
            topic_format="none", filter_keys=[], filter_keys_reg=[]):
        LogtailConfigDetail.__init__(
            self, config_name, logstore_name, endpoint, log_path,
            file_pattern, log_begin_regex, topic_format,
            filter_keys, filter_keys_reg)
...
class LogtailConfigHelper:
    ...
    def generate_common_reg_log_config(json_value):
        ...
        config = CommonRegLogConfigDetail(
            config_name, logstore_name, endpoint, log_path,
            file_pattern, time_format, log_begin_regex, log_parse_regex,
    ...
    def generate_apsara_log_config(json_value):
        ...
        config = ApsaraLogConfigDetail(
            config_name, logstore_name, endpoint, log_path, file_pattern,
    ...
    def generate_logtail_config(json_value):
        if json_value['inputDetail']['logType'] == 'common_reg_log':
            return LogtailConfigHelper.generate_common_reg_log_config(
                json_value)
