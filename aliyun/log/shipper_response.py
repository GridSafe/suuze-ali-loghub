...
class GetShipperConfigResponse(LogResponse):
    def __init__(self, resp, header):
        ...
        if self.type == 'odps':
            ...
            self.config = OssShipperConfig(
                target_config['ossBucket'],
...
class GetShipperTasksResponse(LogResponse):
    def __init__(self, resp, header):
        ...
        for task_res in resp['tasks']:
            task = ShipperTask(
                task_res['id'], task_res['taskStatus'],
                task_res['taskMessage'], task_res['taskCreateTime'],
                task_res['taskLastDataReceiveTime'],
                task_res['taskFinishTime']
            )
