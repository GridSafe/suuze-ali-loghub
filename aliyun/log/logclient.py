...
"""
LogClient class is the main class in the SDK. It can be used
to communicate with log service server to put/get data.
...
class LogClient(object):
    ...
    :param endpoint: log service host name, for example,
                     http://ch-hangzhou.sls.aliyuncs.com
    ...
    def _getHttpResponse(self, method, url, params, body, headers):
        ...
        try:
            ...
            if method.lower() == 'get':
                ...
                r = requests.delete(url, params=params, data=body,
                                    headers=headers, timeout=self._timeout)
            return (r.status_code, r.content, r.headers)
        except Exception as ex:
            raise LogException('LogRequestError', str(ex))

    def _sendRequest(self, method, url, params, body,
        ...
        requestId = header['x-log-requestid'] if 'x-log-requestid'\
            in header else ''
        exJson = None
    ...
    def _send(self, method, project, body, resource, params,
              headers, respons_body_type='json'):
        if body:
        ...
        if self._securityToken and self._securityToken != "":
            headers["x-acs-security-token"] = self._securityToken

        signature = Util.get_request_authorization(
            method, resource, self._accessKey, params, headers)
        headers['Authorization'] = "LOG " + self._accessKeyId + ':' + signature
        url = url + resource
        return self._sendRequest(method, url, params, body,
                                 headers, respons_body_type)
    ...
    def put_logs(self, request):
        ...
        respHeaders = None
        if is_compress:
            respHeaders = self._send('POST', project, compress_data,
                                     resource, params, headers)
        else:
            respHeaders = self._send('POST', project, body,
                                     resource, params, headers)
    ...
    def list_logstores(self, request):
        ...
        resource = '/logstores'
        project = request.get_project()
        (resp, header) = self._send("GET", project, None,
                                    resource, params, headers)
    ...
    def get_logs(self, request):
        ...
        if request.get_line():
            params['line'] = request.get_line()
        if request.get_offset():
            params['offset'] = request.get_offset()
        if request.get_reverse():
            params['reverse'] = 'true' if request.get_reverse() else 'false'
        logstore = request.get_logstore()
        project = request.get_project()
        resource = "/logstores/" + logstore
        (resp, header) = self._send("GET", project, None, resource,
                                    params, headers)
    ...
    def get_cursor(self, project_name, logstore_name, shard_id, start_time):
        ...
        (resp, header) = self._send("GET", project_name, None,
                                    resource, params, headers)
    ...
    def pull_logs(self, project_name, logstore_name, shard_id,
                  cursor, count=1000, end_cursor=None, compress=False):
        ...
        :type count: int
        :param count: the required pull log package count,
                      default 1000 packages
        ...
        if end_cursor and len(end_cursor) > 0:
            params['end_cursor'] = end_cursor
        (resp, header) = self._send("GET", project_name, None,
                                    resource, params, headers, "binary")
        if compress:
    ...
    def create_logstore(self, project_name, logstore_name, ttl, shard_count):
        ...
        (resp, header) = self._send("POST", project_name, body_str,
                                    resource, params, headers)
    ...
    def delete_logstore(self, project_name, logstore_name):
        ...
        (resp, header) = self._send("DELETE", project_name, None,
                                    resource, params, headers)
    ...
    def update_logstore(self, project_name, logstore_name, ttl, shard_count):
        ...
        body["shardCount"] = int(shard_count)
        body_str = json.dumps(body)
        (resp, header) = self._send("PUT", project_name, body_str,
                                    resource, params, headers)
        return UpdateLogStoreResponse(header)

    def list_logstore(self, project_name, logstore_name_pattern=None,
                      offset=0, size=100):
        ...
        :param logstore_name_pattern: the sub name logstore, used for the
                                      server to return logstore names
    ...
    def split_shard(self, project_name, logstore_name, shardId, split_hash):
        ...
        :param split_hash: the internal hash between the
                           shard begin and end hash
        ...
        (resp, header) = self._send("POST", project_name, None,
                                    resource, params, headers)
        return ListShardResponse(resp, header)
    ...
    def merge_shard(self, project_name, logstore_name, shardId):
        ...
        :type shardId: int
        :param shardId : the shard id of the left shard, server
                         will determine the right adjacent  shardId
    ...
    def create_index(self, project_name, logstore_name, index_detail):
        ...
        (resp, header) = self._send("POST", project_name, body,
                                    resource, params, headers)
    ...
    def create_logtail_config(self, project_name, config_detail):
        ...
        :type config_detail: logtail_config_detail.CommonRegLogConfigDetail
                             or logtail_config_detail.ApsaraLogConfigDetail
        :param config_detail: the logtail config detail info,
                              the CommonRegLogConfigDetail is used
                              to create common regex logs ,the
                              ApsaraLogConfigDetail is used to create
                              apsara log
        ...
        headers['x-log-bodyrawsize'] = str(len(body))
        (resp, headers) = self._send("POST", project_name, body,
                                     resource, params, headers)
    ...
    def update_logtail_config(self, project_name, config_detail):
        ...
        :type config_detail: logtail_config_detail.CommonRegLogConfigDetail
                             or logtail_config_detail.ApsaraLogConfigDetail
        :param config_detail: the logtail config detail info, the
                              CommonRegLogConfigDetail is used to create
                              common regex logs, the ApsaraLogConfigDetail
                              is used to create apsara log
        ...
        body = json.dumps(config_detail.to_json())
        headers['x-log-bodyrawsize'] = str(len(body))
        (resp, headers) = self._send("PUT", project_name, body,
                                     resource, params, headers)
    ...
    def delete_logtail_config(self, project_name, config_name):
        ...
        (resp, headers) = self._send("DELETE", project_name, None,
                                     resource, params, headers)
    ...
    def get_logtail_config(self, project_name, config_name):
        ...
        params = {}
        resource = "/configs/" + config_name
        (resp, headers) = self._send("GET", project_name, None,
                                     resource, params, headers)
    ...
    def list_machines(self, project_name, group_name,
                      offset=0, size=100):
    ...
    def apply_config_to_machine_group(self, project_name,
        ...
        (resp, header) = self._send("PUT", project_name, None,
                                    resource, params, headers)
        return ApplyConfigToMachineGroupResponse(header)

    def remove_config_to_machine_group(self, project_name,
                                       config_name, group_name):
    ...
    def _update_acl(self, project_name, logstore_name,
                    acl_action, acl_config):
        headers = {}
        params = {}
        params['type'] = 'acl'
        resource = "/"
        if logstore_name and len(logstore_name) > 0:
            resource = "/logstores/" + logstore_name
    ...
    def update_project_acl(self, project_name, acl_action, acl_config):
        ...
        :param acl_action: "grant" or "revoke", grant or revoke
                           the acl_config to/from a project
    ...
    def update_logstore_acl(self, project_name, logstore_name,
                            acl_action, acl_config):
        ...
        :type acl_action: string
        :param acl_action: "grant" or "revoke", grant or revoke
                           the acl_config to/from a logstore
        ...
        :raise: LogException
        """
        return self._update_acl(project_name, logstore_name,
                                acl_action, acl_config)
    ...
    def list_logstore_acl(self, project_name, logstore_name,
                          offset=0, size=100):
    ...
    def create_shipper(self, project_name, logstore_name,
                       shipper_name, shipper_type, shipper_config):
        ...
        :param shipper_config: the detail shipper config, must be
                                OssShipperConfig or OdpsShipperConfig type
    ...
    def update_shipper(self, project_name, logstore_name,
                       shipper_name, shipper_type, shipper_config):
        ...
        :type shipper_type: string
        :param shipper_type: only support "odps" or "oss", the type must
                             be same with the oringal shipper
    ...
    def list_shipper(self, project_name, logstore_name):
        ...
        params = {}
        resource = "/logstores/" + logstore_name + "/shipper"
        (resp, header) = self._send("GET", project_name, None, resource,
                                    params, headers)
        return ListShipperResponse(resp, header)

    def get_shipper_tasks(self, project_name, logstore_name, shipper_name,
                          start_time, end_time, status_type='',
                          offset=0, size=100):
        ...
        :param status_type : support one of ['', 'fail', 'success', 'running'],
                             if the status_type = '' , return all kinds
                             of status type
    ...
    def retry_shipper_tasks(self, project_name, logstore_name,
                            shipper_name, task_list):
        ...
        :param task_list: the failed task_id list, e.g ['failed_task_id_1',
                          'failed_task_id_2',...], currently the max retry
                          task count 10 every time
